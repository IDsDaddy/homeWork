                    ...
                    <div class="leftbody">
                        ...
                        <section id = "newsMain">
							...
							<div id = "newsTitle">
                                ...
                                News<span style="font-size:18px"> & </span>Recent Events
                            
                            	<span id = "updatedOn">
                                	<?php
										$sthLatestNews = $dbh->prepare("SELECT mdate FROM articles ORDER BY mdate DESC LIMIT 0,1");
										$sthLatestNews->execute();
										$latestMDate = $sthLatestNews->fetch(PDO::FETCH_ASSOC);
										
										$updatedOn = $latestMDate["mdate"];
                            	...
                            	<?php
									...
									if ($currentPage<=$totalPage)
									{
										...
										foreach($theNewsList as $iNewsList)
										{
											...
											if (strtotime($iNewsList['eventDate'])>strtotime(date('Y-m-d h:i:sa')))
											{
												
												echo "<span><img id='upcoming' src='jsImages/upcoming.gif'></span>";
	
											};
														
														
											echo 		
														"<span ><a id='newsListTitle' href='newsitem.php?id=".$iNewsList["id"]."'>".$iNewsList["title"]."<a/></span>
													<br>
														<span id = 'newsListLocation'>Location: <span id = 'newsLocoContent'>".$iNewsList["location"]."</span></span>
													<br>
														<span id = 'newsListETime'>Event Time: <span id = 'newsETContent'>".$iNewsList["eventDate"]."</span></span>
													<br>
														<span><a id='newsListBody' href='newsitem.php?id=".$iNewsList["id"]."'>- ".substr($iNewsList["body"],0,121)."...</a></span>
													<br>
